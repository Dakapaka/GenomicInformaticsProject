...
original_annotation_plot_name = 'spatial_original_annotations_embryo.png'
try:
    if 'annotation_colors' not in adata.uns:
        sc.pl.spatial(adata, color='annotation', title="Original Annotations Embryo", spot_size=spot_size, save=original_annotation_plot_name)
...
pca_result = adata.obsm['X_pca'][:, :10]  # Top 10 PCA components
...
# Function to plot barplot with annotation counts on top
...
# Plot distribution of original annotations using the new function
plot_barplot_with_counts(
    x_labels=unique_orig,
    y_values=counts_orig,
    title='Original Annotation Distribution',
    file_name='original_annotation_distribution.png'
)
...
def plot_confusion_matrix(fold, clf_name, K, L, y_true, y_pred, class_names, title):
    ...
    plt.xticks(rotation=90) 
    plt.title(title)
    plt.savefig(f'confusion_matrix_fold{fold}_{clf_name}_K{K}_L{L}_embryo.png')
...
plt.grid(True)
plt.savefig('figures/pca_explained_variance_percentage.png')
...
for K, L in K_L_combinations:
    ...
    for clf_name, classifier in classifiers.items():
        ...
        for i in range(adata.n_obs):
            ...
            if most_common_pred != y_numeric[i]:
        ...
        refined_annotation_plot_name = f'spatial_refined_annotations_{clf_name}_K{K}_L{L}_embryo.png'
        try:
            sc.pl.spatial(adata, color=f'refined_annotation_{clf_name}_K{K}_L{L}', title=f"Refined Annotations (K={K}, L={L}, {clf_name}) Embryo", spot_size=spot_size, save=refined_annotation_plot_name)
        except Exception as e:
